...
(function() {
  ...
  Altitude.prototype._read = function() {
    ...
    fs.readFileAsync(files.pres_scale, 'utf8')
      .then((s) => {
        pres_scale = parseFloat(s);
        return fs.readFileAsync(files.pres_raw, 'utf8');
      })
      .then((raw) => {
        pressure = parseFloat(raw * pres_scale) * 10;
        return fs.readFileAsync(files.temp_scale, 'utf8');
      })
      .then((s) => {
        temp_scale = parseFloat(s);
        return fs.readFileAsync(files.temp_raw, 'utf8');
      })
      .then((raw) => {
        temperature = parseFloat(raw * temp_scale);
        altitude = ( ( ( ( Math.pow( ( P0 / pressure ), ( 1/5.257 ) ) ) - 1 ) * ( temperature + 273.15 ) ) / 0.0065 );
